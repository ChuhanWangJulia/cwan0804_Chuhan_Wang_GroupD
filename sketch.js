...
function setup() {
  createCanvas(800, 800);
   ...
   circles = [
    ...
    //The smaller circles surrounding the larger circle.
    //The small circle with three centers.
    {
...
function drawTheseCircles() {
  for (let i = 0; i < circles.length; i++) {
    ...
    if (circle.dotCounts && circle.radiusFactor && circle.dotColors) {
      drawDotsPerlinNoise(circle.x, circle.y, circle.size, circle.dotCounts, circle.radiusFactor, circle.dotColors);
...
function drawDotsPerlinNoise(x, y, diameter, dotCounts, radiusFactor, dotColors) {
  ...
  for (let ring = 0; ring < numRings; ring++) {
    ...
    for (let dot = 0; dot < numDots; dot++) {
      ...
      //Adjust the smoothness of the noise effect.
      let noiseFactor = noise(dot * 0.2, frameCount * 0.06);
      //Adjust the offset range of noise dynamic effect.
      let offsetRadius = radius + noiseFactor * 22 - 10;
      let dotX = x + offsetRadius * cos(angle);
      let dotY = y + offsetRadius * sin(angle);
